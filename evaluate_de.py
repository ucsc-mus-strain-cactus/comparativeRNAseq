"""
Generate intersection comparison tables for each genome
"""
import sys
import os
...
for base_path, dirs, files in os.walk(ref_dir):
    if files:
        try:
            ...
            continue
        h = open(os.path.join(base_path, 'RSEM.genes.results'))
        _ = h.next()  # skip header
        for l in h:
            gene_id, tx_ids, length, eff_length, exp_count, tpm, fpkm = l.split()
            ref_data_map[genome][experiment][gene_id] = exp_count
