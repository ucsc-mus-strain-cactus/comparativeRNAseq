...
recs = recs[1:]
for x in recs:
    biotype_map[x[0]] = gencode.get(x[2], 'Other')



biotype_s = pd.Series(biotype_map, name='Biotype')


tm_dir = '/hive/users/ifiddes/ihategit/pipeline/mouse_work_v4/C57B6J/GencodeCompVM8/transMap'
from collections import Counter



def load_paralogy(g, df):
    r = Counter()
    for l in open(os.path.join(tm_dir, g + '.gp')):
        l = l.rstrip().split('\t')
        ens_id = l[0].split('-')[0]
        r[ens_id] += 1
    gene_r = {gene_id: False for gene_id in df.index}
    for ens_id, c in r.iteritems():
        if c > 1:
            gene_id = tx_map[ens_id]
            gene_r[gene_id] = True
...
db = '/hive/users/ifiddes/ihategit/pipeline/mouse_work_v4/C57B6J/GencodeCompVM8/comparativeAnnotator/classification.db'
from comparativeAnnotator.database_queries import get_fail_pass_excel_ids, get_gene_transcript_map
from comparativeAnnotator.comp_lib.name_conversions import strip_alignment_numbers
gene_map = get_gene_transcript_map('C57B6J', db)


def load_excel_good(g, df):
    excel_ids, pass_specific_ids, fail_ids = get_fail_pass_excel_ids('C57B6J', g, db)
    # strip alignment numbers
    excel_ids = {strip_alignment_numbers(x) for x in excel_ids}
    pass_specific_ids = {strip_alignment_numbers(x) for x in pass_specific_ids}
    fail_ids = {strip_alignment_numbers(x) for x in fail_ids}
    r = {}
    for gene_id in df.index:
        c = Counter()
        for tx in gene_map[gene_id]:
            if tx in excel_ids:
                c["Excellent"] += 1
            elif tx in pass_specific_ids:
                c['Passing'] += 1
            elif tx in fail_ids:
                c['Fail'] += 1
        # pick most common category
        r[gene_id] = sorted(c.iteritems(),key=lambda x: (-x[1], x[0]))[0][0]
    return df.join(pd.Series(r, name='Classification'), how='inner')
...
for g in genomes:
    ...
    df = df[df['ref'] >= 1]
    df = df[df['tgt'] >= 1]
    num_sig = len([x for x in df['Unnamed: 2'] if x <= 0.05])
    num_non_sig = len(df) - num_sig
    df['ref'] = map(log, df['ref'])
    df['tgt'] = map(log, df['tgt'])
    p_val_vector = ['<= 0.05 (n = {})'.format(num_sig) if x <= 0.05 else '> 0.05 (n = {})'.format(num_non_sig) for x in df['Unnamed: 2']]
    df['Unnamed: 2'] = p_val_vector
    ...
    p.set(xlim=(0, 12), ylim=(0, 12))
    p.savefig('scatterplots/{}_nonzero_log.pdf'.format(g), format='pdf')
    df = df.join(biotype_s, how='inner')
    p2 = sns.lmplot(x=ref_col_name, y=tgt_col_name, data=df, scatter_kws={"s": 6, "alpha": 0.7}, col='Biotype', col_wrap=2, hue='Adjusted p-value', fit_reg=False)
    p2.set(xlim=(0, 12), ylim=(0, 12))
    p2.savefig('scatterplots/{}_nonzero_log_biotype.pdf'.format(g), format='pdf')
    df2 = load_paralogy(g, df)
    p3 = sns.lmplot(x=ref_col_name, y=tgt_col_name, data=df2, scatter_kws={"s": 6, "alpha": 0.7}, col='Paralogy', hue='Adjusted p-value', fit_reg=False)
    p3.set(xlim=(0, 12), ylim=(0, 12))
    p3.savefig('scatterplots/{}_nonzero_log_biotype_paralogy.pdf'.format(g), format='pdf')
    df3 = load_excel_good(g, df)
    p4 = sns.lmplot(x=ref_col_name, y=tgt_col_name, data=df3, scatter_kws={"s": 6, "alpha": 0.7}, col='Classification', hue='Adjusted p-value', fit_reg=False)
    p4.set(xlim=(0, 12), ylim=(0, 12))
    p4.savefig('scatterplots/{}_nonzero_log_biotype_classification.pdf'.format(g), format='pdf')


# now lets try coloring this by source method.
base_dir = '/hive/users/ifiddes/ihategit/pipeline/mouse_output_v4/combined_gene_sets'
for g in genomes:
    p = os.path.join(base_dir, g, 'combined.gp')
    d = {}
    for l in open(p):
        l = l.rstrip().split('\t')
        ens_id = l[0]
        ...
        assert False
        if 'aug' in aln_id:
            d[ens_id] = 'AugustusTMR'
        elif 'jg' in aln_id:
            d[ens_id] = 'AugustusCGP'
        elif 'g' in aln_id and g in ['PWK_PhJ', 'SPRET_EiJ', 'CAST_EiJ']:
            d[ens_id] = 'AugustusPacBio'
        else:
            d[ens_id] = 'transMap'


# this doesn't easily work because we can have multiple source methods per gene. need to do some sort of color blending.
